...
public class IPO {
    public int findMaximizedCapital(int k, int w, int[] profits, int[] capital) {
        ...
        PriorityQueue<int[]> capMinHeap = new PriorityQueue<>((a, b) -> Integer.compare(a[0], b[0]));
        ...
        for (int i = 1; i <= k; i++) {
            ...
            w += profit;
            while (capMinHeap.size() > 0 && capMinHeap.peek()[0] <= w) {
                int[] pollVal = capMinHeap.poll();
                profitMaxHeap.add(pollVal[1]);
            }
        }
