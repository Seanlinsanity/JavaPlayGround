...
class Twitter {
    class Post {
        int time;
        int id;
        Post(int _id, int _time) {
            this.id = _id;
            this.time = _time;
        }
    }

    Map<Integer, List<Post>> userPostMap;
    ...
    public void postTweet(int userId, int tweetId) {
        userPostMap.putIfAbsent(userId, new ArrayList<>());
        userPostMap.get(userId).add(new Post(tweetId, time));
        time += 1;
    }
    ...
    public List<Integer> getNewsFeed(int userId) {
        ...
        userIds.add(userId);
        Set<Integer> followIds = userFollowMap.get(userId);
        if (followIds != null) {
        ...
        PriorityQueue<Post> maxHeap = new PriorityQueue<>((a, b) -> Integer.compare(b.time, a.time));
        for (int user: userIds) {
            List<Post> posts = userPostMap.get(user);
            if (posts == null) { continue; }

            int startIndex = 0;
            if (posts.size() > 10) {
                startIndex = posts.size() - 10;
            }

            for (int i = startIndex; i < posts.size(); i++) {
                maxHeap.add(posts.get(i));
            }
        }

        List<Integer> result = new ArrayList<>();
        while (result.size() < 10 && maxHeap.size() > 0) {
