...
public class ChildrenSumInBinaryTree {
    public static int isSumProperty(TreeNode root)
    {
        ...
        return result != Integer.MIN_VALUE ? 1 : 0;
    }

    static int postOrderTraverse(TreeNode node) {
        if (node == null) {
            return 0;
        }
        ...
        int leftVal = postOrderTraverse(node.left);
        int rightVal = postOrderTraverse(node.right);
